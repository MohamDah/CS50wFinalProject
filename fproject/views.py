...
def index(request):
    ...
    for f in friends:
        try:
            u1_messages = message.objects.filter(message_sender=request.user, message_reciever=f)
        except:
            u1_messages = []
    
        try:
            u2_messages = message.objects.filter(message_sender=f, message_reciever=request.user)
        except:
            u2_messages = []
        ...
        if messages == None:
            continue
        messages = messages.order_by("-date").first()
        recents.append(messages)
...
def chat(request, username):
    ...
    if u2 not in u1.friends.all():
        return render(request, "fproject/apology.html", {
